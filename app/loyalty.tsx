...
// "Insufficient points", never a generic "unknown error".
...
export default function Loyalty() {
  ...
  // "Earn by purchase amount": points = amount × ratio, computed CLIENT-SIDE
  // because the API takes points, not currency (spec §5). The ratio and symbol
  // come from brand.json so each merchant configures their own scheme.
  const amountNum = Number.parseFloat(amount);
  const points = Number.isFinite(amountNum)
    ? Math.floor(amountNum * brand.loyalty.pointsPerCurrencyUnit)
    : 0;
  const idOk = MEMBER_ID.test(memberId);
  const valid = idOk && points >= 1;
  ...
  const run = (m: typeof earn) => {
    ...
    redeem.reset();
    setLookup(true);
    m.mutate({ memberId, input: { points, reason: reason.trim() || undefined } });
  };
    ...
    <ScrollView style={{ backgroundColor: colors.bg }}>
      <Screen>
        <Card>
          ...
          {member.data ? (
            <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
              <Muted>balance</Muted>
              <Body color={colors.good}>{member.data.account.balance} pts</Body>
            </View>
          ) : member.error ? (
        ...
        <Card>
          ...
          <Field
            label={`Purchase amount (${brand.loyalty.currencySymbol})`}
            value={amount}
          ...
          />
          <Muted>
            = {points} pts at {brand.loyalty.pointsPerCurrencyUnit} pt per{" "}
            {brand.loyalty.currencySymbol}.
          </Muted>
        ...
        {earn.data || redeem.data ? (
          <Card>
            ...
            <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
              ...
              <Body>{(earn.data ?? redeem.data)!.account.balance} pts</Body>
            </View>
          </Card>
        ) : null}
